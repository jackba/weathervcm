...
Ext.ux.ItemSelector = Ext.extend(Ext.form.Field,  {
    ...
    onRender: function(ct, position){
		...
		if (!this.readOnly) {
			if (!this.toSortField) {
				...
				this.upIcon.on('click', this.reset, this);
				//this.downIcon.on('click', this.down, this);
				this.downIcon.on('click', this.addAll, this);
	...
	addAll : function() {
		//var selectionsArray = this.fromMultiselect.view.getSelectedIndexes();
		var selectionsArray = this.fromMultiselect.view.store.getRange();
		var records = [];
		if (selectionsArray.length > 0) {
			for (var i=0; i<selectionsArray.length; i++) {
				record = this.fromMultiselect.view.store.getAt(selectionsArray[i]);
				records.push(selectionsArray[i]);
			}
			if(!this.allowDup)selectionsArray = [];
			for (var i=0; i<records.length; i++) {
				record = records[i];
