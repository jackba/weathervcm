...
<body>
<div class="wrap">
	<h1>当前位置：系统资源&nbsp;&gt;&nbsp;<span class="position_current"> 用户使用次数排行榜</span></h1>
		<div id="searchArea" class="search">
			<div id="queryArea" style="display:none">
			<form name="form1" id="form1" method="post" action="<%=request.getContextPath()%>/stat_searchUserReserveStat.do">
...
//	alert(Ext.get('status').dom.value);
	...
	if(mychart==null || mychart==undefined){
		mychart = new Ext.ux.Chart.Fusion.Panel({
       				...
       				dataURL  : '<%=request.getContextPath()%>/stat_searchUserReserveStat.do?startDate='+Ext.get("startDate").dom.value+'&endDate='+Ext.get("endDate").dom.value,
		...
		mychart.setChartDataURL('<%=request.getContextPath()%>/stat_searchUserReserveStat.do?startDate='+Ext.get("startDate").dom.value+'&endDate='+Ext.get("endDate").dom.value,true);
		//mychart.refreshMedia();
	}
}
function reset() {
