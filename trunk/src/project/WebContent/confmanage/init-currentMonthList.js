...
function initData() {
	ds = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'conf_searchCurrentMonths.do'
		}),
...
function initGrid() {
	...
	var cm = new Ext.grid.ColumnModel([{
		...
		renderer : function(value, p , record){
			...
			var action = "window.parent.createNewPanel('confReserveDetail_{0}','currentMonthConf','会议详情','conf_reserveDetail.do?personal=false%26conferenceId={1}');";
	...
	grid = new Ext.grid.GridPanel({
		...
		tbar : ['<b>&nbsp;&nbsp;&nbsp;&nbsp;<font color=#990000>本月会议安排</font></b>','->', {
			id : 'btnPdf',
			text : '生成PDF报表',
			pressed : true,
			tooltip : '生成PDF报表',
...
function loadStore(start){
	ds.load({
		params : {
			...
			'virtualConfId' : Ext.get("virtualConfIdk").dom.value,
			'listType' : 'currentMonth'
		}
	});
}
