...
function initData() {
	ds = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'conf_searchCurrentWeeks.do'
		}),
...
function initGrid() {
	...
	var cm = new Ext.grid.ColumnModel([sm, {
		...
		renderer : function(value, p , record){
			...
			var action = "window.parent.createNewPanel('confReserveDetail_{0}','currentWeekConf','会议详情','conf_reserveDetail.do?personal=false%26conferenceId={1}');";
	...
	grid = new Ext.grid.GridPanel({
		...
		tbar : ['<b>&nbsp;&nbsp;&nbsp;&nbsp;<font color=#990000>本周会议安排</font></b>','->',{
			id : 'btnMonitor',
			text : '监控',
			pressed : true,
			tooltip : '会议监控',
