...
Ext.onReady(function(){
	...
	var tds = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'conftemplate_getUnitsByConfTemplateId.do?confTemplateId=<%=request.getParameter("confTemplateId")%>&selected=true'
		}),
		reader : new Ext.data.JsonReader({
			root : 'root'
		}, [{
			...
			success:function(result,request){
				...
				if(resp.success == true){	
					Ext.Msg.alert('成功',resp.msg, function(button){
						if(button == 'ok'){
							location.href = '<%=request.getContextPath() %>/conftemplate_list.do' ;
							window.parent.closeAndRefreshPanel("conftemplateModify_<s:property value='confTemplate.confTemplateId'/>");
